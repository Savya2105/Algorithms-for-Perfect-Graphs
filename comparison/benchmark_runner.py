...
FIXES APPLIED:
1. Theta selection: Uses max(candidates) instead of or logic
2. CSV export: Pre-computes strings before f-strings
"""
...
class BenchmarkRunner:
        ...
        try:
            ...
            try:
                ...
                if theta_exact is not None:
                    ...
                    print(f" Exact small graphs: No known exact value")
                    theta_exact = None
            except Exception as e:
                theta_exact = None
            ...
            # FIXED: Theta selection using max() instead of or logic
            candidates = []
            if theta_exact is not None:
                candidates.append(('exact', theta_exact))
            if theta_eigen is not None:
                candidates.append(('eigen', theta_eigen))
            if theta_simple is not None:
                candidates.append(('simple', theta_simple))

            if candidates:
                # Pick the MAXIMUM theta value to satisfy ω(G) ≤ θ(G)
                best_method, theta_best = max(candidates, key=lambda x: x[1])
            else:
                theta_best = None
    ...
    def print_csv_export(self, filename="benchmark_results.csv"):
        ...
        FIXED: Pre-computes strings before f-strings to avoid ValueError.
        """
        if not self.results:
        ...
        with open(filename, 'w') as f:
            ...
            # Rows - FIXED: Pre-compute strings OUTSIDE format specifiers
            for r in self.results:
                # Pre-compute all strings to avoid ValueError
                theta_str = f"{r['theta_best']:.6f}" if r['theta_best'] is not None else "N/A"
                num_colors_str = str(r['num_colors']) if r['num_colors'] is not None else "N/A"
                theta_time_str = f"{r['time_theta_total']*1000:.3f}" if r['time_theta_total'] is not None else "N/A"
                coloring_time_str = f"{r['time_coloring']*1000:.3f}" if r['time_coloring'] is not None else "N/A"
